...
class _ChatScreenState extends State<ChatScreen> {
  ...
  Widget _item_row_1(MessageModel message) {
    //发送给我的
    return Container(
  ...
  Widget _item_row_2(MessageModel message) {
    // 我发送的
    return Container(
        ...
        child: Row(
          ...
          children: <Widget>[
            ...
            Expanded(
              child: new Wrap(
                // 受 row 影响  需要用弹性布局 包装Container 才能根据文字自适应小
                textDirection: TextDirection.rtl,
                children: <Widget>[
                  Container(
                    decoration: BoxDecoration(
  ...
  Widget _item_row_3(MessageModel message) {
    // 系统的等等++
    return Container(
  ...
  Widget _item_row_time(MessageModel message) {
    // 时间分割
    return Container(
  ...
  Widget _itemBuilder(c, i) {
    // item 未封装 后期为节省时间 以后暂时都不封装
    List<Widget> rows = [];

    if (i % 3 == 0) {
      // 判断2个时间戳是否相隔 N 分钟 超过就添加时间分割Widget
    ...
    setState(() {
      msgs = List.generate(10, (i) {
        ...
        return new MessageModel(
            name: "昵称${type}",
            avatar: "https://picsum.photos/128/128?image=${type}",
            msg: "我是内容哦我是内容哦我是内容哦我是内容哦我是内容哦我是内容哦我是内容哦我是内容哦",
            type: type);
      }).toList();
  ...
  _add_new() {
    // 添加最新消息
    List<MessageModel> msgs2 = List.generate(10, (i) {
      // 测试二
  ...
  Widget _widget_appBar_bottom() {
    ...
    return PreferredSize(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
            color: Colors.white,
            border: BorderDirectional(top: BorderSide(width: 0.2))),
      ),
    );
  }
